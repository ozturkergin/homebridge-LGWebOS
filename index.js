...
class webosTvDevice {
  ...
  prepareInputSourcesService() {
    ...
    // append statically configured inputs to available inputs
    this.inputs.forEach((staticInput) => {
      let existing = availableInputs.find(i => i.appId === staticInput.appId);
      if (existing) {
        existing.name = staticInput.name; // update name
      } else {
        availableInputs.push(staticInput);
      }
      
      // also ensure statically configured inputs are visible by default if not set otherwise
      if (!this.tvInputsConfig[staticInput.appId]) {
        this.tvInputsConfig[staticInput.appId] = { visible: true, name: staticInput.name };
        try {
           fs.writeFileSync(this.tvInputConfigFile, JSON.stringify(this.tvInputsConfig));
        } catch (e) {
           this.logDebug('Error writing tvInputsConfig in prepareInputSourcesService');
        }
      }
    });
  ...
  initInputSources() {
    ...
    // append statically configured inputs to tv inputs
    this.inputs.forEach((staticInput) => {
      let existing = tvInputList.find(i => i.appId === staticInput.appId);
      if (existing) {
        existing.name = staticInput.name;
      } else {
        tvInputList.push(staticInput);
      }
    });
